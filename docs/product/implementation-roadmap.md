...
**Current status, verified at `f6791a4`:** B Open · C Open · D Open · E Open ·
...
| Language match by primary subtag (content, `inLanguage`) | Implemented | `langMatches` at `src/integrations/seo-lint/lint.ts:176-185`, used at `routes.ts:266-285`; test `lint.test.ts` (es vs es-MX) | Mutation: comparing full tags instead of primary subtags failed `lintHtml — JSON-LD inLanguage cross-check (LD_LANG_MISMATCH) > passes when primary subtag matches across regional variants (es vs es-MX)` (`expected true to be false`), plus `lintLocaleRoutes > passes when every indexable page matches the locale in its URL` and the regional hreflang coverage test; restored |
| Regional variants in hreflang alternate coverage | Not built | `routes.ts:153-165` compares `hreflang` by exact string against `LOCALES` | Mutation: exact-only hreflang matching failed `lintLocalizedRouteCoverage > accepts regional hreflang variants by primary language subtag` with `expected [...] to deeply equal []`; restored |
| Asset classification by emitted inventory | Implemented | `walkAssets` in `src/integrations/seo-lint/index.ts:95-111`, consumed at `routes.ts:366-400`; tests `routes.test.ts:429-493` (file-shaped routes, dotted slugs, Unicode names) | Mutation: excluding extensionless emitted files from `walkAssets` exposed a coverage gap; added `walkAssets > includes extensionless emitted files in the asset inventory`, which failed with `expected [] to deeply equal ['/downloads/NOTICE']`; restored |
