...
**Current status:** B Open (verified at `f6791a4`) · C Open (reopened at
`7bd6f1e`) · D Open · E Open · F Open · G Open.
...
**Status at `7bd6f1e`: Open** — reopened after an adversarial review. The suite is green
(`bun run test` 485/485, `bun run check` 0 errors, `bun run build` with seo-lint clean,
10 pages), but none of the inputs below is covered, so the gates are not proven.

| Deliverable | Status | Evidence | Missing proof |
|---|---|---|---|
| Order- and quote-insensitive attribute extraction | Implemented | `extractTags` / `decodeMarkupValue` in `src/integrations/seo-lint/routes.ts:56-96`, read by `declaredCanonical`, `declaredOgUrl`, `declaredAlternates`, `isIndexable`, `parseSitemap`; tests `routes.test.ts` ("extracts canonical links with reordered uppercase attributes, single quotes and entities", og:url, robots, sitemap variants); commit `50e25e4` | — (mutation recorded: restoring the fixed-order double-quote canonical regex failed the reordered/single-quote/entity test with `expected null to be '…?a=1&b=2'`; restored) Open: a quoted `>` inside an attribute value (`href="…?q=a>b"`) truncates the tag and empties the canonical, so every canonical gate skips it; markup inside comments, `<script>` and `<template>` is read as effective; an out-of-range numeric entity (`&#9999999999;`) throws `RangeError` instead of producing a finding. |
| Language match by primary subtag (content, `inLanguage`) | Implemented | `langMatches` at `src/integrations/seo-lint/lint.ts:181-186`, used by `lintLocaleRoutes` at `routes.ts:436`; test `lint.test.ts` (es vs es-MX) | — (mutation recorded: comparing full tags instead of primary subtags failed `lintHtml — JSON-LD inLanguage cross-check (LD_LANG_MISMATCH) > passes when primary subtag matches across regional variants (es vs es-MX)` (`expected true to be false`), plus `lintLocaleRoutes > passes when every indexable page matches the locale in its URL` and the regional hreflang coverage test; restored) |
| Regional variants in hreflang alternate coverage | Implemented | `alternateProblems` in `routes.ts:198-230`: exact match first, primary-subtag fallback only for a bare configured locale no other locale names exactly; tests `routes.test.ts` ("accepts regional hreflang variants by primary language subtag", "keeps configured regional locales of one language distinct", "still reports a genuinely missing locale when another locale is regional"); commit `50e25e4` | — (mutations recorded: exact-only matching failed `lintLocalizedRouteCoverage > accepts regional hreflang variants by primary language subtag` with `expected [...] to deeply equal []`; a primary-subtag-only match failed the distinct-regional test with `2 hreflang="es-ES" entries`; restored) Open: malformed tags such as `es-` or `es-NOT_A_TAG` satisfy coverage for `es`, because the primary-subtag fallback never validates the tag. |
...
| Sitemap discovery opt-out | Implemented | Independent `sitemap` / `noindex` content controls (schema rule in `src/lib/content/discovery.ts`); explicit HTML exclusion marker under a strict three-state contract (`sitemapMarkerState` in `routes.ts:289-335`: `none` / `valid` / `invalid`); marker-only `sitemap-opt-out` postprocessor, which only ever acts on `valid`; `SITEMAP_PAGE_MISSING` / `SITEMAP_OPTED_OUT_PAGE` / `SITEMAP_NOINDEX_PAGE` / `SITEMAP_MARKER_INVALID` gates and tests; commits `f6fead0`, `e3cb2d8`, `d5f9e65` | — (mutations recorded: ignoring the marker → `SITEMAP_PAGE_MISSING`; treating any omission as intentional → accidental-omission test red (`[]`); postprocessor off → `SITEMAP_OPTED_OUT_PAGE` on both locales; removing the coming-soon marker → `SITEMAP_NOINDEX_PAGE` en/es; `noindex: true` with default `sitemap` → schema refine failure; restoring case/trim folding → 5 named marker tests red; real build with `content="Exclude"` on coming-soon → exit 1 with `SITEMAP_MARKER_INVALID` + `SITEMAP_NOINDEX_PAGE` on `/coming-soon/` and `/es/coming-soon/`, no `SITEMAP_PAGE_MISSING`; all restored) Open: a marker inside nested `<template>` elements is classified `valid`, so the postprocessor drops the URL with no finding. |
...
| Initialiser value-flow provenance | Implemented | `documented-codes.test.ts` follows const/let initialisers, simple aliases, and named object-literal properties; virtual graph tests cover each boundary; commit `454a424` | — (mutation recorded: fixture produced `ALIASED_UNDOCUMENTED_CODE`; both documentation parity assertions named it) Open: a code returned by a local function that wraps a factory from an excluded `node_modules` module reaches a collector with no declaration and no failure. |
